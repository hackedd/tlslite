...
class TLSRecordLayer:
			...
			while len(self._readBuffer)<min and not self.closed:
				try:
					for result in self._getMsg(
						...
						if result in (0,1) or isinstance(result, HelloRequest) or isinstance(result, ClientHello):
							yield result
							continue;
					applicationData = result
					self._readBuffer += bytesToString(applicationData.write())
				except TLSRemoteAlert, alert:
					if alert.description == AlertDescription.unrecognized_name:
						self.received_SNI_alert =True
					elif alert.description != AlertDescription.close_notify:
						raise
	...
	def writeAsync(self, s):
		...
		try:
			if not self._application_enabled:
				if not self._application_queue:
					self._application_queue = s
				else:
					self._application_queue += s
				return
			elif self._application_queue:
				s = self._application_queue + s
				self._application_queue = None
			...
			while 1:
				...
				for result in self._sendMsg(applicationData, skipEmptyFrag, collect_records):
					yield result
	...
	def _sendMsg(self, msg, skipEmptyFrag=True, collect_records = False):
		...
		if contentType == ContentType.handshake:
			...
			self._handshake_sha256.update(bytesStr)
		...
		if self._writeState.encContext:
			...
			if self._writeState.encContext.isBlockCipher:
				...
				if self.extra_padding_bytes:
					...
					assert self.extra_padding_bytes >=0
		...
		r = RecordHeader3().create(self.rec_version, contentType, len(bytes))
		self.rec_version = self.version
	...
	def _getMsg(self, expectedType, secondaryType=None, constructorType=None):
		try:
			...
			original_expectedType = expectedType
			if self._application_enabled:
				expectedType += (ContentType.application_data, )
			...
			while 1:
						...
						if alert.level == AlertLevel.warning or \
						   alert.description == AlertDescription.close_notify:
							...
							if alert.description == \
								...
								self._shutdown(True)
							elif alert.description == AlertDescription.unrecognized_name:
								pass; # do nothing
						...
						if self._client:
							if subType == HandshakeType.hello_request:
								if self._handshake_in_progress:
									continue; #Ignore hello_requests while in handshake
								reneg = True
						else:
							if subType == HandshakeType.client_hello:
								reneg = True
						#Send no_renegotiation, then try again
						if reneg and not self.renegotiationAllowed:
							alertMsg = Alert()
				...
				if recordHeader.type == ContentType.change_cipher_spec:
					...
					elif subType == HandshakeType.client_hello:
	...
	def _calcPendingStates(self, clientRandom, serverRandom, implementations):
		...
		if self.session.cipherSuite in CipherSuite.aes128Suites:
			...
			keyLength = 16
			ivLength = 16
			if self.session.cipherSuite in CipherSuite.Sha256Suites:
				macLength = 32
				digest_module = sha256
		...
		if self.version == (3,0):
			...
			keyBlock = PRF_1_2(self.session.masterSecret,
						   "key expansion",
						   concatArrays(serverRandom,clientRandom),
						   outputLength)
		else:
	...
	def _sendFinished(self):
		...
		for result in self._sendMsg(ChangeCipherSpec(), collect_records=True):
			yield result
	...
	def _getFinished(self):
		...
		for result in self._getMsg(ContentType.change_cipher_spec):
			if result in (0,1) or isinstance(result, ApplicationData):
				yield result
	...
	def _calcFinished(self, send=True):
		if self.version == (3,0):
			...
			handshakeHashes = stringToBytes(self._handshake_sha256.digest())
			verifyData = PRF_1_2(self.session.masterSecret, label, handshakeHashes, 12)
			return verifyData
		else:
			raise AssertionError()
	...
	def EnableFalseStart(self,flag):
		self.do_false_start = flag
